...
string s[]={"(A)","(B)","(C)","(D)","(E)","(F)"};
    string a[]={"ACov","BCov","CCov","DCov","ECov","FCov"};
    int i,j;
    float marks=0;
    string ans[6];
    for(i=0;i<6;i++)
    ...
    for(i=0;i<6;i++)
    {
        for(j=0;j<6;j++)
        {
            if(ans[j].find(s[i])<ans[j].size() && ans[j].find(a[i])<ans[j].size())
            {
                marks+=1;
                break;
            }
            else if(ans[j].find(s[i])<ans[j].size() || ans[j].find(a[i])<ans[j].size())
            {
                marks+=0.5;
                break;
            }
