...
const long long int MOD = 1e9+7;//(119<<23)+1==998244353
template<typename T>
struct FenwickTree
{
    T N;
...
void dfs(int u,int v = 0 ){
    ...
    for(auto child : adj[u]){
         dfs(child,u);
        ans[u]= max(ans[u],ans[child]);
        subs[u]+=subs[child];
    }
    ans[u]+=subs[u];

}
/*ll fin(int u,int par){
    if((int)adj[u].size() == 0) //If leaf
      return subs[u];
...
void solve(){
    int n;
    cin>>n;
    for(int i = 0 ; i<=n ;i ++) adj[i].clear(),subs[i]=0,ans[i]=0;
        //ans = 0 ;
    for(int i = 2 ; i<=n ;i++){
        cin>>num;
        adj[num].push_back(i);
