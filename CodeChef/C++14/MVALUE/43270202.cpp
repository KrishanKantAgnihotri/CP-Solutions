...
void solve(){
    ...
    ll sec =v[0];
    for(int i = 1 ;i<n-1 ;i++){
                if(one*v[i]+max(one-v[i],v[i]-one)>(one*sec+max(one-sec,sec-one))){
    ...
    ll sec1 =v[n-1];
    for(int i = 1 ;i<n-1 ;i++){
        if(one1*v[i]+max(one1-v[i],v[i]-one1)>(one1*sec1+max(one1-sec1,sec1-one1))){
            sec1 = v[i];
        }
    }
    
    cout<<(max(one*sec+max(one-sec,sec-one),one1*sec1+max(one1-sec1,sec1-one1)))<<endl;
