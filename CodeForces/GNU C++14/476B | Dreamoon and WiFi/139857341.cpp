...
ll calc(ll n,ll one,ll two){
    if(n == 0){
        if(one == two) return 1;
        else return 0 ; 
    }
    return calc(n-1,one+1,two)+calc(n-1,one-1,two);
}
bool test = false;
bool file = true;
void solve(){
    ...
    cin>>a>>b;
    if(a == b){
        cout<<setprecision(11)<<fixed<<double(1*1.0)<<endl;
    }
    else{
        ll n = count(all(b),'?');
        ll one = sol(a);
        ll two = sol(b);
        ll val = calc(n,one,two);
        cout<<setprecision(11)<<fixed<<double(val/double(pow(2,n)))<<endl;
    }
}
int main(){
    if(file)
