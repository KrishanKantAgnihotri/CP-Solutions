...
while(t--)
{
    ...
    ll ans = 0 ; 
    for(ll tot = 0 ;tot<=s;tot++){
        ll cntsi = min(p/cs,tot);
        ll cntai = min(a,(p-cntsi*cs)/ca);
        ll cntsu = min((s-cntsi),f/cs);
        ll cntau = min((a-cntai),(f-cntsu*cs)/ca);
        ans = max(cntsi+cntai+cntsu+cntau,ans);
    }
    cout<<ans<<endl;
}
}
