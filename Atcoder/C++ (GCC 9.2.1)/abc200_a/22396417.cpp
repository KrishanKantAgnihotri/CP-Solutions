...
void solve(){
    ...
    for(int j = 1 ;j<n ;j++){
        for(int k = j+1 ;k<n;k++){
            ll mx = max({v[0].a,v[j].a,v[k].a});
           ll mx1 = max({v[0].b,v[j].b,v[k].b});
           ll mx2 = max({v[0].c,v[j].c,v[k].c});
           ll mx3 = max({v[0].d,v[j].d,v[k].d});
           ll mx4 = max({v[0].e,v[j].e,v[k].e});
           ans  = max(ans,min({mx,mx1,mx2,mx3,mx4}));
        }
    }
    sort(all(v),cmp1);
     for(int j = 1 ;j<n ;j++){
