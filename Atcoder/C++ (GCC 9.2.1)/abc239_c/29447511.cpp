...
ll a,b,c,d;
cin>>a>>b>>c>>d;
vector<ll> x({a,c,a-1,c-1,a-2,c-2,a-5,c-5,a+1,c+1,a+2,c+2,a+5,c+5});
vector<ll> y({b,d,b-1,d-1,b-2,d-2,b-5,d-5,b+1,d+1,b+2,d+2,b+5,d+5});
for(int i = 0 ;i<16 ;i++){
    for(int j = 0 ;j<16 ;j++){
        ll cur1 = (a-x[i])*(a-x[i]) + (b-y[j])*(b-y[j]);
        ll cur2 = (c-x[i])*(c-x[i]) + (d-y[j])*(d-y[j]);
        if(cur1 == 5 && cur2 == 5){
            cout<<"Yes\n";
            return ;
        }
